...
var initResponsiveBackgroungImages = function initResponsiveBackgroungImages($) {
	...
	window.responsiveBackgroundImages = function (selector) {
		...
		$bg_img_el.each(function (index, element) {
			...
			img_current_src = $(element).css(css_prop).replace("url(", "").replace(")", "").replace(/\"/gi, "");
			...
			$.each(this.attributes, function (index, element) {
				...
				if (this.name.indexOf('data-bg-') !== -1) {
					...
					// have a match.
					// -------------------------------------
					if (this.value !== '' && typeof this.value !== 'undefined' && typeof match[1] !== 'undefined') {
			...
			for (var i = 0; i < img_data.length; i++) {
				...
				if (Modernizr.mq('screen and ( min-width: ' + bp_min + 'px ) and ( max-width: ' + bp_max + 'px )')) {
					...
					// conditions.
					// -------------------------------------
					if (img_current_src !== src && (retina === false && pixelRatio < 2 || retina === true && pixelRatio > 1)) {
			...
			// reason that may be.
			// -------------------------------------
			var bg_img = $img_target.css(css_prop).replace("url(", "").replace(")", "").replace(/\"/gi, "");

			if (bg_img === 'none' && img_default_src !== '') {
